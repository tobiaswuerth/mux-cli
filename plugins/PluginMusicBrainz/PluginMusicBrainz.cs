...
namespace ch.wuerth.tobias.mux.plugins.PluginMusicBrainz
{
    public class PluginMusicBrainz : PluginBase
    {
        ...
        public PluginMusicBrainz(LoggerBundle logger) : base("musicbrainz"
            , logger)
        {
        ...
        protected override void Process(String[] args)
        {
            ...
            do
            {
                ...
                using (DataContext context =
                    new DataContext(new DbContextOptions<DataContext>()
                        , Logger))
                {
                    data = context.SetMusicBrainzRecords
                        .Where(x => null == x.LastMusicBrainzApiCall)
                        .Include(x => x.MusicBrainzAliasMusicBrainzRecords)
                        .ThenInclude(x => x.MusicBrainzAlias)
                        .Include(x
                            => x.MusicBrainzArtistCreditMusicBrainzRecords)
                    ...
                    Logger?.Information?.Log(
                        $"Batch containing: {data.Count} entries");
                        ...
                        try
                        {
                            ...
                            switch (o)
                            {
                                ...
                                case JsonErrorMusicBrainz err:
                                    mbr.MusicBrainzApiCallError =
                                        err.Error?.Trim() ?? "<unknown>";
                                    Logger?.Exception?.Log(
                                        new MusicBrainzApiException(
                                            $"Error: {mbr.MusicBrainzApiCallError}"));
                                    break;
                            }
        ...
        private void HandleResponse(MusicBrainzRecord mbr
            , JsonMusicBrainzRequest json
            , DataContext context)
        {
            ...
            List<MusicBrainzAlias> aliases =
                json.Aliases?.Select(x => MusicBrainzMapper.Map(context, x))
                    .ToList()
                ?? new List<MusicBrainzAlias>();

            List<MusicBrainzAliasMusicBrainzRecord> aliasRecord = aliases
                .Select(x => new MusicBrainzAliasMusicBrainzRecord
                {
            ...
            mbr.MusicBrainzAliasMusicBrainzRecords = mbr
                .MusicBrainzAliasMusicBrainzRecords.Concat(aliasRecord.Where(x
                    => !mbr.MusicBrainzAliasMusicBrainzRecords.Any(y
                        => y.MusicBrainzAliasUniqueId.Equals(
                                x.MusicBrainzAliasUniqueId)
                            && y.MusicBrainzRecordUniqueId.Equals(
                                x.MusicBrainzRecordUniqueId))))
                .ToList();

            // artist credits
            List<MusicBrainzArtistCredit> credits =
                json.ArtistCredit
                    ?.Select(x => MusicBrainzMapper.Map(context, x))
                    .ToList()
                ?? new List<MusicBrainzArtistCredit>();

            List<MusicBrainzArtistCreditMusicBrainzRecord> artistCreditRecords =
                credits.Select(x
            ...
            mbr.MusicBrainzArtistCreditMusicBrainzRecords = mbr
                .MusicBrainzArtistCreditMusicBrainzRecords.Concat(
                    artistCreditRecords.Where(x
                        => !mbr.MusicBrainzArtistCreditMusicBrainzRecords.Any(y
                            => y.MusicBrainzArtistCreditUniqueId.Equals(
                                    x.MusicBrainzArtistCreditUniqueId)
                                && y.MusicBrainzRecordUniqueId.Equals(
                                    x.MusicBrainzRecordUniqueId))))
                .ToList();
            ...
            List<MusicBrainzRelease> releases =
                json.Releases
                    ?.Select(x => MusicBrainzMapper.Map(context, x, Logger))
                    .ToList()
                ?? new List<MusicBrainzRelease>();

            List<MusicBrainzReleaseMusicBrainzRecord> releaseRecords = releases
                .Select(x => new MusicBrainzReleaseMusicBrainzRecord
                {
                    MusicBrainzRecord = mbr
                    , MusicBrainzRecordUniqueId = mbr.UniqueId
            ...
            mbr.MusicBrainzReleaseMusicBrainzRecords = mbr
                .MusicBrainzReleaseMusicBrainzRecords.Concat(
                    releaseRecords.Where(x
                        => !mbr.MusicBrainzReleaseMusicBrainzRecords.Any(y
                            => y.MusicBrainzReleaseUniqueId.Equals(
                                    x.MusicBrainzReleaseUniqueId)
                                && y.MusicBrainzRecordUniqueId.Equals(
                                    x.MusicBrainzRecordUniqueId))))
                .ToList();

            // tags
            List<MusicBrainzTag> tags =
                json.Tags?.Select(x => MusicBrainzMapper.Map(context, x))
                    .ToList()
            ...
            mbr.MusicBrainzTagMusicBrainzRecords = mbr
                .MusicBrainzTagMusicBrainzRecords.Concat(tagRecords.Where(x
                    => !mbr.MusicBrainzTagMusicBrainzRecords.Any(y
                        => y.MusicBrainzTagUniqueId.Equals(
                                x.MusicBrainzTagUniqueId)
                            && y.MusicBrainzRecordUniqueId.Equals(
                                x.MusicBrainzRecordUniqueId))))
                .ToList();
        }
